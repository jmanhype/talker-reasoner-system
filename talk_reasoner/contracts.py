...
def _validate_event(event: LedgerEvent) -> None:
    checks = (
        ...
        (event.rejection_reason is None or not re.search(r"(?i)(?:raw transcript|raw_content|transcript:)", event.rejection_reason),
         "rejection_reason", "must not expose raw or transcript content"),
        (event.rejection_reason is None or not any(pattern.search(event.rejection_reason) for pattern in CREDENTIAL_PATTERNS),
         "rejection_reason", "must not expose credential-like values"),
