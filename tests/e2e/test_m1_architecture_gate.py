...
def test_complete_m1_architecture_gate_emits_pass_report() -> None:
    ...
    conditions = {
        ...
        "python_and_ts_checked": metrics["python"] == 8 and metrics["ts"] == 4,
        "oracle_tests_scanned": metrics["tests"] == 13 and metrics["machines"] == 7 and metrics["rows"] == 62,
        "exact_denominators": len(oracles) == 7 and len(transitions) == 62 and len(invariants) == 36,
        "suite_collected": collection_exit == 0 and collected_tests == 338,
